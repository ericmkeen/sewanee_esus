...
#### (4) Produce a plot comparing the distribution of environmental risk exposure (`RISK SCORE`) across census tracts in California (CA) vs. Florida (FL). Note that state abbreviations are in the column `ST_ABBR`. 
...
Produce a plot that displays the relationship between a census tract's whiteness (`PCT.WHITE_2016`) and its cancer risk (`cancer_risk`).  Set the y-axis range to 0-70.
...
#### (9) Before we plot anything, filter the dataset to rows where `whiteness` is not `NA` and `cancer_risk` is less than 100.  
...
#### (10) Now compare the two whiteness categories using a boxplot (use `geom_boxplot()`).
